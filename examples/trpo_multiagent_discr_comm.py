...
from rllab.envs.multi_agent_discr_comm_grid_world_guided_env import MultiAgentDiscrCommGridWorldGuidedEnv
from rllab.envs.normalized_env import normalize
from sandbox.rocky.tf.envs.base import TfEnv
from sandbox.rocky.tf.policies.multi_agent_discr_msg_policy import MultiAgentDiscreteMsgMLPPolicy
...
parser.add_argument('--no-swapobs', dest='swapobs', action='store_false')
parser.set_defaults(swapobs=True)
...
parser.set_defaults(collision=False)
args = parser.parse_args()
##

prefix = '../temp/discr_comm/'
if args.swapobs:
    prefix = prefix + 'swap_obs/'
else:
    prefix = prefix + 'no_swap/'
...
env = TfEnv(normalize(MultiAgentDiscrCommGridWorldGuidedEnv(n = n_agent,
                                                   desc = map_desc,
                                                   collision = args.collision,
                                                   swap_goal_obs = args.swapobs,
                                                   has_comm_msg = args.msgdim > 0)))

policy = MultiAgentDiscreteMsgMLPPolicy(
    'MAP',
    n_row,
    n_col,
    n_agent,
    env_spec=env.spec,
    feature_dim = 15, # feature from each agent's local information
    msg_dim = args.msgdim, # when msgdim == 0, no communication
    conv_layers=[], # number of conv-layers and the number of kernels
    hidden_layers=[64, 32, 32], # hidden layers after conv-layers
    comm_layers = [15], # hidden layers after receiving msgs from other agents
